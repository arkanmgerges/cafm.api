        ...
        if doNotSkip:
            ...
            if not skipGeneratingFile:
                ...
                _print(
                    ...
                    message=f"{modelProtoName}.proto",
                    innerDepth=1,
                )
            renderedTemplate = modelAppTemplate.render(
            ...
            if not skipGeneratingFile:
                ...
                _print(
                    modelName=f'{model["name"]}',
                    message=f"{modelProtoName}_app_service.proto",
                    innerDepth=1,
                )
        if isGenerated:
            _print(
...
def generateCommandConstant():
    ...
    for modelConfig in Config.configData["domain_model"]:
        ...
        doNotSkip = (
            ...
            if (
                "skip" in model
                and "command_constant" not in model["skip"]
                and "all" not in model["skip"]
            )
            or ("skip" not in model)
            else False
        ...
        if doNotSkip:
            _addTemplateBeforeSignatureEnd(
                fullFilePath=f"{messageFullPath}/common/model/CommandConstant",
...
def generateRoute():
    ...
    for modelConfig in Config.configData["domain_model"]:
        ...
        if doNotSkip:
            ...
            renderedTemplate = testTemplate.render(
                model=model, segment=Config.configData["segment"]
            )
            if ("file_overwrite" not in model) or (
                "file_overwrite" in model and model["file_overwrite"] is False
                ...
                if _isManuallyModified(
                    fileFullPath=f"{modelName}.py", templateString=renderedTemplate
                ):
                    _print(
                        modelName="",
            ...
            _print(
                modelName=f'{model["name"]}',
                message=f"generating {modelName} for #modelName",
                innerDepth=1,
            )
        if isGenerated:
...
def generateRouterModelResponse():
    ...
    for modelConfig in Config.configData["domain_model"]:
        ...
        doNotSkip = (
            ...
            if (
                "skip" in model
                and "model_response" not in model["skip"]
                and "all" not in model["skip"]
            )
            or ("skip" not in model)
            else False
        ...
        if doNotSkip:
            _print(
                modelName=f'{model["name"]}',
                message=f"generating files for #modelName",
                innerDepth=1,
            )
            ...
            if not skipGeneratingFile:
                ...
                _print(
                    modelName=f'{model["name"]}',
                    message=f"{modelTestName}.py for #modelName",
                    innerDepth=2,
                )
            ...
            if ("file_overwrite" not in model) or (
                ...
                if _isManuallyModified(
                    fileFullPath=f"{modelTestName}s.py", templateString=renderedTemplate
                ):
                    _print(
                        modelName="",
            ...
            if not skipGeneratingFile:
                ...
                _print(
                    modelName=f'{model["name"]}',
                    message=f"{modelTestName}s.py for #modelName",
                    innerDepth=2,
                )
...
def generateGrpcApiClient():
    ...
    for modelConfig in Config.configData["domain_model"]:
        ...
        doNotSkip = (
            ...
            if (
                "skip" in model
                and "grpc_client" not in model["skip"]
                and "all" not in model["skip"]
            )
            or ("skip" not in model)
            else False
        ...
        if doNotSkip:
            ...
            if ("file_overwrite" not in model) or (
                ...
                if _isManuallyModified(
                    fileFullPath=f"{fullPathModelName}Client.py",
                    templateString=renderedTemplate,
                ):
                    _print(
                        modelName="",
            ...
            _print(
                modelName=f'{model["name"]}',
                message=f"{fullPathModelName}Client.py for #modelName",
                innerDepth=1,
            )
                ...
                if fileLines[signatureStartIndex].find(signatureStart) != -1:
                    for signatureEndIndex in range(
                        ...
                        if fileLines[signatureEndIndex].find(signatureEnd) != -1:
                            fileLines.insert(
                                signatureEndIndex - 1, f"{renderedTemplate}\n"
                            )
                            break
                    break
...
def funcArgsLowerCamelCaseJinjaFilter(
    value, objectName=None, objectType=None, sign="="
):
    if objectName is not None:
        if objectType == "function":
            res = map(
