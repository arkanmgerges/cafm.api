...
class UserClient(Client):
    ...
    def users(self, resultFrom: int = 0, resultSize: int = 10) -> List[User]:
        with grpc.insecure_channel(f'{self._server}:{self._port}') as channel:
            ...
            try:
                ...
                response: UserAppService_usersResponse = stub.users.with_call(
                    UserAppService_usersRequest(resultFrom=resultFrom, resultSize=resultSize),
                    metadata=(('token', self.token),))
                logger.debug(
                    f'[{UserClient.users.__qualname__}] - grpc response: {response}')

                return [User(id=user.id, name=user.name) for user in response[0].users]
