...
from src.resource.proto._generated.project.daily_check_procedure_app_service_pb2 import \
    ...
    DailyCheckProcedureAppService_newIdRequest, DailyCheckProcedureAppService_newIdResponse
...
class DailyCheckProcedureClient(Client):
    ...
    def newId(self) -> str:
        with grpc.insecure_channel(f'{self._server}:{self._port}') as channel:
            ...
            try:
                ...
                response: DailyCheckProcedureAppService_newIdResponse = stub.newId.with_call(
                    request,
                    metadata=(('token', self.token), (
                        'opentel', AppDi.instance.get(OpenTelemetry).serializedContext(DailyCheckProcedureClient.newId.__qualname__))))
                logger.debug(
    ...
    @OpenTelemetry.grpcTraceOTel
    def dailyCheckProceduresByEquipmentOrGroupId(self, equipmentOrGroupId: str = None, resultFrom: int = 0, resultSize: int = 10, order: List[dict] = None) -> DailyCheckProcedures:
        order = [] if order is None else order
        with grpc.insecure_channel(f'{self._server}:{self._port}') as channel:
            stub = DailyCheckProcedureAppServiceStub(channel)
            try:
                ...
                request = DailyCheckProcedureAppService_dailyCheckProceduresByEquipmentOrGroupIdRequest(equipmentOrGroupId=equipmentOrGroupId, resultFrom=resultFrom, resultSize=resultSize)
                [request.order.add(orderBy=o["orderBy"], direction=o["direction"]) for o in order]
