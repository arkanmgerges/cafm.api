...
from src.port_adapter.api.rest.model.response.v1.project.Organization import Organization
...
from src.port_adapter.api.rest.model.response.v1.project.UserLookup import UserLookupDescriptor
from src.port_adapter.api.rest.model.response.v1.project.UserLookups import UserLookups
from src.resource.logging.logger import logger
from src.resource.logging.opentelemetry.OpenTelemetry import OpenTelemetry
from src.resource.proto._generated.project.user_lookup_app_service_pb2 import UserLookupAppService_userLookupsRequest, \
    UserLookupAppService_userLookupsResponse
from src.resource.proto._generated.project.user_lookup_app_service_pb2_grpc import UserLookupAppServiceStub
...
class UserLookupClient(Client):
    ...
    def userLookups(self, resultFrom: int = 0, resultSize: int = 10, order: List[dict] = None) -> UserLookups:
        ...
        with grpc.insecure_channel(f'{self._server}:{self._port}') as channel:
            ...
            try:
                ...
                request = UserLookupAppService_userLookupsRequest(resultFrom=resultFrom, resultSize=resultSize)
                [request.order.add(orderBy=o["orderBy"], direction=o["direction"]) for o in order]
                response: UserLookupAppService_userLookupsResponse = stub.userLookups.with_call(
                    request,
                ...
                return UserLookups(
                    user_lookups=[self._descriptorByObject(obj=userLookup) for userLookup in response[0].userLookups],
    ...
    def _descriptorByObject(self, obj: Any) -> UserLookupDescriptor:
        return UserLookupDescriptor(user=User(id=obj.user.id,
