...
from src.resource.proto._generated.project.equipment_input_app_service_pb2 import \
    ...
    EquipmentInputAppService_equipmentInputsByEquipmentIdRequest, EquipmentInputAppService_equipmentInputsByEquipmentIdResponse, \
...
class EquipmentInputClient(Client):
    ...
    @OpenTelemetry.grpcTraceOTel
    def equipmentInputsByEquipmentId(self, equipmentId: str = None, resultFrom: int = 0, resultSize: int = 10, order: List[dict] = None) -> EquipmentInputs:
        order = [] if order is None else order
        with grpc.insecure_channel(f'{self._server}:{self._port}') as channel:
            stub = EquipmentInputAppServiceStub(channel)
            try:
                ...
                request = EquipmentInputAppService_equipmentInputsByEquipmentIdRequest(equipmentId=equipmentId, resultFrom=resultFrom, resultSize=resultSize)
                [request.order.add(orderBy=o["orderBy"], direction=o["direction"]) for o in order]
