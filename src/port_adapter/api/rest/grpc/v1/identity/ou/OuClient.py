        ...
        with grpc.insecure_channel(f'{self._server}:{self._port}') as channel:
            ...
            try:
                ...
                response: OuAppService_ousResponse = stub.ous.with_call(
                    request,
                    metadata=(('token', self.token), ('opentel', AppDi.instance.get(OpenTelemetry).serializedContext(OuClient.ous.__qualname__))))
                logger.debug(
        ...
        with grpc.insecure_channel(f'{self._server}:{self._port}') as channel:
            ...
            try:
                ...
                response: OuAppService_ouByIdResponse = stub.ouById.with_call(
                    ...
                    metadata=(('token', self.token), ('opentel', AppDi.instance.get(OpenTelemetry).serializedContext(OuClient.ouById.__qualname__))))
                logger.debug(
