...
from src.resource.proto._generated.identity.permission_context_app_service_pb2 import \
    ...
    PermissionContextAppService_newIdRequest, PermissionContextAppService_newIdResponse
    ...
    def newId(self) -> str:
        with grpc.insecure_channel(f'{self._server}:{self._port}') as channel:
            ...
            try:
                ...
                response: PermissionContextAppService_newIdResponse = stub.newId.with_call(
                    request,
                    metadata=(('token', self.token), (
                        'opentel', AppDi.instance.get(OpenTelemetry).serializedContext(PermissionContextClient.newId.__qualname__))))
                logger.debug(
