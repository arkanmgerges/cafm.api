    ...
    def citiesByStateId(
        ...
        with grpc.insecure_channel(f"{self._server}:{self._port}") as channel:
            ...
            try:
                ...
                request = CityAppService_citiesByStateIdRequest(
                    id=stateId, resultFrom=resultFrom, resultSize=resultSize
                )
                [
                    request.order.add(orderBy=o["orderBy"], direction=o["direction"])
                    for o in order
                ]
                response: CityAppService_citiesByStateIdResponse = (
                    stub.citiesByStateId.with_call(
                        request, metadata=(("token", self.token),)
                    )
                )
                logger.debug(
