...
async def getSubcontractorsBySubcontractorCategoryId(
    *,
    subcontractor_category_id: str = Path(
        ...,
        description="The subcontractor category id that is used to fetch subcontractor category data",
    ),
    result_from: int = Query(0, description="Starting offset for fetching data"),
    result_size: int = Query(10, description="Item count to be fetched"),
    order: str = Query("", description="e.g. id:asc,email:desc"),
...
@router.get(
    ...
    summary="Get subcontractors by id",
...
async def createSubcontractor(
    *,
    ...
    website_url: str = Body(
        ..., description="The website url of the subcontractor", embed=True
    ),
    contact_person: str = Body(
        ..., description="The contact person of the subcontractor", embed=True
    ),
    email: str = Body(..., description="Email of the subcontractor", embed=True),
    phone_number: str = Body(
        ..., description="Phone number of the subcontractor", embed=True
    ),
    subcontractor_category_id: str = Body(
        ..., description="The category of the subcontractor", embed=True
    ),
    address_one: str = Body(..., description="subcontractor first address", embed=True),
    address_two: str = Body(
        ..., description="subcontractor second address", embed=True
    ),
):
    ...
    producer.produce(
        obj=ProjectCommand(
            id=reqId,
            name=CommandConstant.CREATE_SUBCONTRACTOR.value,
...
@router.put(
    "/{subcontractor_id}",
    summary="Update a subcontractors",
...
async def updateSubcontractor(
    *,
    ...
    producer.produce(
        obj=ProjectCommand(
            id=reqId,
            name=CommandConstant.UPDATE_SUBCONTRACTOR.value,
            metadata=json.dumps({"token": Client.token}),
            data=json.dumps(
                {
...
@router.patch(
    "/{subcontractor_id}",
    summary="Partial update a subcontractors",
    status_code=status.HTTP_200_OK,
)
@OpenTelemetry.fastApiTraceOTel
async def partialUpdateSubcontractor(
    *,
    ...
    website_url: str = Body(
        None, description="The website url of the subcontractor", embed=True
    ),
    contact_person: str = Body(
        None, description="The contact person of the subcontractor", embed=True
    ),
    email: str = Body(None, description="Email of the subcontractor", embed=True),
    phone_number: str = Body(
        None, description="Phone number of the subcontractor", embed=True
    ),
    subcontractor_category_id: str = Body(
        None, description="The category of the subcontractor", embed=True
    ),
    address_one: str = Body(
        None, description="subcontractor first address", embed=True
    ),
    address_two: str = Body(
        None, description="subcontractor second address", embed=True
    ),
):
    reqId = RequestIdGenerator.generateId()
    producer = AppDi.instance.get(SimpleProducer)
...
@router.delete(
    "/{subcontractor_id}",
    summary="Delete a subcontractors",
    status_code=status.HTTP_200_OK,
)
@OpenTelemetry.fastApiTraceOTel
async def deleteSubcontractor(
    *,
    ...
    producer.produce(
        obj=ProjectCommand(
            id=reqId,
            name=CommandConstant.DELETE_SUBCONTRACTOR.value,
            metadata=json.dumps({"token": Client.token}),
...
@OpenTelemetry.fastApiTraceOTel
async def assignSubcontractorToOrganization(
    *,
    _=Depends(CustomHttpBearer()),
    subcontractor_id: str = Path(
        ...,
    ...
    producer.produce(
        obj=ProjectCommand(
            id=reqId,
            name=CommandConstant.ASSIGN_SUBCONTRACTOR_TO_ORGANIZATION.value,
            metadata=json.dumps({"token": Client.token}),
            data=json.dumps(
                {
                    "subcontractor_id": subcontractor_id,
...
async def revokeAssignmentSubcontractorToOrganization(
    *,
    ...
    producer.produce(
        obj=ProjectCommand(
            id=reqId,
            name=CommandConstant.REVOKE_ASSIGNMENT_SUBCONTRACTOR_TO_ORGANIZATION.value,
            metadata=json.dumps({"token": Client.token}),
            data=json.dumps(
                {
                    "subcontractor_id": subcontractor_id,
