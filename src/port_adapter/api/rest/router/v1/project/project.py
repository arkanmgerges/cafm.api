...
@OpenTelemetry.fastApiTraceOTel
async def getProjectsByOrganizationId(
    *,
    organization_id: str = Path(
        ..., description="Organization id that is used to fetch project data"
    ),
    result_from: int = Query(0, description="Starting offset for fetching data"),
    result_size: int = Query(10, description="Item count to be fetched"),
    orders: str = Query("", description="e.g. name:asc,age:desc"),
    ...
    try:
        ...
        orderService = AppDi.instance.get(OrderService)
        orders = orderService.orderStringToListOfDict(orders)
        return client.projectsByOrganizationId(
        ...
        if e.code() == StatusCode.NOT_FOUND:
            ...
            logger.error(
                f"[{getProjectsByOrganizationId.__module__}.{getProjectsByOrganizationId.__qualname__}] - error response e: {e}"
            )
            return Response(content=str(e), status_code=HTTP_500_INTERNAL_SERVER_ERROR)
    except Exception as e:
        logger.info(e)
    ...
    *,
    ...
    try:
        ...
        return client.projectsByState(
            state=state, resultFrom=result_from, resultSize=result_size, orders=orders
        )
    except grpc.RpcError as e:
        if e.code() == StatusCode.PERMISSION_DENIED:
            return Response(content=str(e), status_code=HTTP_403_FORBIDDEN)
...
async def createBuildingLevel(
    *,
    ...
    is_sub_level: bool = Body(None, description="Is it a sub level", embed=True),
):
    reqId = RequestIdGenerator.generateId()
    producer = AppDi.instance.get(SimpleProducer)
    client = ProjectClient()
    is_sub_level = is_sub_level if is_sub_level is not None else False
    producer.produce(
        obj=ProjectCommand(
            id=reqId,
