...
from src.port_adapter.api.rest.grpc.v1.project.equipment.input.EquipmentInputClient import (
    EquipmentInputClient,
)
...
@router.get(
    path="", summary="Get all equipment input(s)", response_model=EquipmentInputs
)
...
@router.get(
    ...
    summary="Get all equipment input(s) by equipment id",
    response_model=EquipmentInputs,
)
@OpenTelemetry.fastApiTraceOTel
async def getEquipmentInputsByEquipmentId(
    *,
    equipment_id: str = Path(
        ..., description="equipment id that is used to fetch equipment input data"
    ),
    result_from: int = Query(0, description="Starting offset for fetching data"),
    result_size: int = Query(10, description="Item count to be fetched"),
    order: str = Query("", description="e.g. id:asc,email:desc"),
    ...
    try:
        ...
        orderService = AppDi.instance.get(OrderService)
        order = orderService.orderStringToListOfDict(order)
        return client.equipmentInputsByEquipmentId(
...
@router.get(
    path="/{equipment_input_id}",
    summary="Get equipment input by id",
    response_model=EquipmentInputDescriptor,
)
@OpenTelemetry.fastApiTraceOTel
async def getEquipmentInputById(
    *,
...
async def createEquipmentInput(
    *,
    ...
    equipment_id: str = Body(
        ..., description="equipment id of equipment input", embed=True
    ),
):
    ...
    producer.produce(
        obj=ProjectCommand(
            id=reqId,
            name=CommandConstant.CREATE_EQUIPMENT_INPUT.value,
...
@router.put(
    "/{equipment_input_id}",
    summary="Update equipment input",
...
async def updateEquipmentInput(
    *,
    ...
    producer.produce(
        obj=ProjectCommand(
            id=reqId,
            name=CommandConstant.UPDATE_EQUIPMENT_INPUT.value,
            metadata=json.dumps({"token": Client.token}),
            data=json.dumps(
                {
...
@router.patch(
    "/{equipment_input_id}",
    summary="Partial update equipment input",
    status_code=status.HTTP_200_OK,
)
@OpenTelemetry.fastApiTraceOTel
async def partialUpdateEquipmentInput(
    *,
    ...
    equipment_id: str = Body(
        None, description="equipment id of equipment input", embed=True
    ),
):
    reqId = RequestIdGenerator.generateId()
...
@router.delete(
    "/{equipment_input_id}",
    summary="Delete a equipment inputs",
    status_code=status.HTTP_200_OK,
)
@OpenTelemetry.fastApiTraceOTel
async def deleteEquipmentInput(
    *,
    ...
    producer.produce(
        obj=ProjectCommand(
            id=reqId,
            name=CommandConstant.DELETE_EQUIPMENT_INPUT.value,
            metadata=json.dumps({"token": Client.token}),
            data=json.dumps({"equipment_input_id": equipment_input_id}),
            external=[],
        ),
        schema=ProjectCommand.get_schema(),
    )
