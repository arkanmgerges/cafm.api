...
from src.port_adapter.api.rest.grpc.v1.project.equipment.category.EquipmentCategoryClient import (
...
@router.get(
    path="", summary="Get all equipment category(s)", response_model=EquipmentCategories
)
...
async def getEquipmentCategories(
    *,
    ...
    try:
        ...
        return client.equipmentCategories(
...
@router.get(
    path="/{equipment_category_id}",
    summary="Get equipment category by id",
    response_model=EquipmentCategoryDescriptor,
)
@OpenTelemetry.fastApiTraceOTel
async def getEquipmentCategoryById(
    *,
    equipment_category_id: str = Path(
        ...,
...
@OpenTelemetry.fastApiTraceOTel
async def createEquipmentCategory(
    *,
    _=Depends(CustomHttpBearer()),
    ...
    producer.produce(
        obj=ProjectCommand(
            id=reqId,
            name=CommandConstant.CREATE_EQUIPMENT_CATEGORY.value,
...
@OpenTelemetry.fastApiTraceOTel
async def updateEquipmentCategory(
    *,
    _=Depends(CustomHttpBearer()),
    equipment_category_id: str = Path(
        ...,
        description="equipment category id that is used in order to update the equipment category",
    ),
    ...
    producer.produce(
        obj=ProjectCommand(
            id=reqId,
            name=CommandConstant.UPDATE_EQUIPMENT_CATEGORY.value,
            metadata=json.dumps({"token": Client.token}),
            data=json.dumps(
                {
...
@router.delete(
    "/{equipment_category_id}",
    summary="Delete a equipment categories",
    status_code=status.HTTP_200_OK,
)
@OpenTelemetry.fastApiTraceOTel
async def deleteEquipmentCategory(
    *,
    _=Depends(CustomHttpBearer()),
    equipment_category_id: str = Path(
        ...,
        description="equipment category id that is used in order to delete the equipment category",
    ),
):
    reqId = RequestIdGenerator.generateId()
    producer = AppDi.instance.get(SimpleProducer)
    ...
    producer.produce(
        obj=ProjectCommand(
            id=reqId,
            name=CommandConstant.DELETE_EQUIPMENT_CATEGORY.value,
            metadata=json.dumps({"token": Client.token}),
            data=json.dumps({"equipment_category_id": equipment_category_id}),
            external=[],
        ),
        schema=ProjectCommand.get_schema(),
    )
...
async def getCategoryGroupsByCategoryId(
    *,
    ...
    try:
        ...
        orderService = AppDi.instance.get(OrderService)
        order = orderService.orderStringToListOfDict(order)
        return client.categoryGroupsByCategoryId(
