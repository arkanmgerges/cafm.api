...
from src.port_adapter.api.rest.grpc.v1.project.standard_equipment.standard_category.StandardEquipmentCategoryClient import StandardEquipmentCategoryClient
from src.port_adapter.api.rest.model.response.v1.project.standard_equipment.standard_category.StandardEquipmentCategories import StandardEquipmentCategories
from src.port_adapter.api.rest.model.response.v1.project.standard_equipment.standard_category.StandardEquipmentCategory import StandardEquipmentCategoryDescriptor
...
async def createStandardEquipmentCategory(*, _=Depends(CustomHttpBearer()),
                 name: str = Body(..., description='name of standard equipment category', embed=True),
                ):
    reqId = str(uuid4())
...
@router.put("/{standard_equipment_category_id}", summary='Update standard equipment category', status_code=status.HTTP_200_OK)
@OpenTelemetry.fastApiTraceOTel
async def updateStandardEquipmentCategory(*, _=Depends(CustomHttpBearer()),
                 standard_equipment_category_id: str = Path(..., description='standard equipment category id that is used in order to update the standard equipment category'),
...
@router.patch("/{standard_equipment_category_id}", summary='Partial update standard equipment category', status_code=status.HTTP_200_OK)
@OpenTelemetry.fastApiTraceOTel
...
@router.delete("/{standard_equipment_category_id}", summary='Delete a standard equipment categories', status_code=status.HTTP_200_OK)
@OpenTelemetry.fastApiTraceOTel
async def deleteStandardEquipmentCategory(*, _=Depends(CustomHttpBearer()),
                 standard_equipment_category_id: str = Path(..., description='standard equipment category id that is used in order to delete the standard equipment category'), ):
    reqId = str(uuid4())
    producer = AppDi.instance.get(SimpleProducer)
    from src.port_adapter.messaging.common.model.ProjectCommand import ProjectCommand
