...
@router.get(
    path="",
    summary="Get all standard equipment category(s)",
    response_model=StandardEquipmentCategories,
)
...
async def getStandardEquipmentCategories(
    *,
    ...
    try:
        ...
        return client.standardEquipmentCategories(
...
@router.get(
    path="/{standard_equipment_category_id}",
    summary="Get standard equipment category by id",
    response_model=StandardEquipmentCategoryDescriptor,
)
@OpenTelemetry.fastApiTraceOTel
async def getStandardEquipmentCategoryById(
    *,
...
@router.post(
    "", summary="Create standard equipment category", status_code=status.HTTP_200_OK
)
@OpenTelemetry.fastApiTraceOTel
async def createStandardEquipmentCategory(
    *,
    ...
    producer.produce(
        obj=ProjectCommand(
            id=reqId,
            name=CommandConstant.CREATE_STANDARD_EQUIPMENT_CATEGORY.value,
...
@router.put(
    ...
    summary="Update standard equipment category",
...
async def updateStandardEquipmentCategory(
    *,
    ...
    producer.produce(
        obj=ProjectCommand(
            id=reqId,
            name=CommandConstant.UPDATE_STANDARD_EQUIPMENT_CATEGORY.value,
            metadata=json.dumps({"token": Client.token}),
            data=json.dumps(
                {
...
@router.patch(
    "/{standard_equipment_category_id}",
    summary="Partial update standard equipment category",
    status_code=status.HTTP_200_OK,
)
@OpenTelemetry.fastApiTraceOTel
...
@router.delete(
    "/{standard_equipment_category_id}",
    summary="Delete a standard equipment categories",
    status_code=status.HTTP_200_OK,
)
@OpenTelemetry.fastApiTraceOTel
async def deleteStandardEquipmentCategory(
    *,
    ...
    producer.produce(
        obj=ProjectCommand(
            id=reqId,
            name=CommandConstant.DELETE_STANDARD_EQUIPMENT_CATEGORY.value,
            metadata=json.dumps({"token": Client.token}),
            data=json.dumps(
                {"standard_equipment_category_id": standard_equipment_category_id}
            ),
