...
async def getCities(*,
    ...
    try:
        ...
        return client.cities(resultFrom=result_from, resultSize=result_size, order=order)
...
@router.get(path="/{state_id}/cities", summary='Get a state cities', response_model=Cities)
@OpenTelemetry.fastApiTraceOTel
async def getCitiesByStateId(*, result_from: int = Query(0, description='Starting offset for fetching data'),
                             result_size: int = Query(10, description='Item count to be fetched'),
                             order: str = Query('', description='e.g. name:asc,age:desc'),
                             state_id: str = Path(...,
    ...
    try:
        ...
        orderService = AppDi.instance.get(OrderService)
        order = orderService.orderStringToListOfDict(order)
        return client.citiesByStateId(stateId=state_id, resultFrom=result_from, resultSize=result_size,
                                      order=order)
    except grpc.RpcError as e:
        if e.code() == StatusCode.PERMISSION_DENIED:
            return Response(content=str(e), status_code=HTTP_403_FORBIDDEN)
